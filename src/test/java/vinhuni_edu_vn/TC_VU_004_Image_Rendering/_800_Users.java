...
/**
 * TC_VU_004: Image Rendering Test - 800 Users
 * 800 người dùng truy cập trang có nhiều hình ảnh
 * Expected: Thời gian phản hồi trung bình < 18s, tỷ lệ thành công > 65%
 */
public class _800_Users extends Simulation {
...

  {
    ...
    .assertions(
      global().responseTime().mean().lte(18000),
      global().responseTime().percentile3().lte(35000),
      global().successfulRequests().percent().gte(65.0)
    );
  }
}
