...
/**
 * TC_VU_001: Homepage Load Test - 100 Users
 * 100 người dùng truy cập trang chủ đồng thời
...
public class _100_Users extends Simulation {
  ...
  private ScenarioBuilder homepageLoadScenario = scenario("Homepage Load Test - 100 Users")
    .exec(
  ...
  {
    setUp(
      // Test với tổng 100 users: ramp up trong 1 phút
      homepageLoadScenario.injectOpen(
        rampUsers(100).during(Duration.ofMinutes(1))
      )
    ).protocols(httpProtocol)
    .assertions(
      global().responseTime().mean().lte(8000), // Thời gian phản hồi trung bình < 8s
      global().responseTime().percentile3().lte(15000), // 95% < 15s
      global().successfulRequests().percent().gte(75.0) // 75% thành công
    );
  }
}
