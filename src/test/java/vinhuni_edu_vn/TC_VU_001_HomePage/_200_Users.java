...
/**
 * TC_VU_001: Homepage Load Test - 200 Users
 * 200 người dùng truy cập trang chủ đồng thời
 * Expected: Thời gian phản hồi trung bình < 10s, tỷ lệ thành công > 70%
 */
public class _200_Users extends Simulation {
  ...
  private ScenarioBuilder homepageLoadScenario = scenario("Homepage Load Test - 200 Users")
    .exec(
  ...
  {
    setUp(
      // Test với tổng 200 users: ramp up trong 2 phút
      homepageLoadScenario.injectOpen(
        rampUsers(200).during(Duration.ofMinutes(1))
      )
    ).protocols(httpProtocol)
    .assertions(
      global().responseTime().mean().lte(10000), // Thời gian phản hồi trung bình < 10s
      global().responseTime().percentile3().lte(20000), // 95% < 20s
      global().successfulRequests().percent().gte(70.0) // 70% thành công
    );
  }
}
