package viblo_asia.TC_VIBLO_DYNAMIC;

import io.gatling.javaapi.core.*;
import io.gatling.javaapi.http.*;

import static io.gatling.javaapi.core.CoreDsl.*;
import static io.gatling.javaapi.http.HttpDsl.*;

import java.time.Duration;
import java.util.Map;
import java.util.List;

/**
 * Dynamic Viblo Category Filter Test
 * Accepts user count via system property: -DuserCount=N
 */
public class Dynamic_Category_Filter_Test extends Simulation {

    // Get user count from system property, default to 100
    private static final int USER_COUNT = Integer.parseInt(System.getProperty("userCount", "100"));

    // HTTP configuration
    private HttpProtocolBuilder httpProtocol = http
        .baseUrl("https://viblo.asia")
        .acceptHeader("text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8")
        .acceptLanguageHeader("en-US,en;q=0.5")
        .acceptEncodingHeader("gzip, deflate")
        .userAgentHeader("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36");

    // Category filters feeder
    private FeederBuilder<Object> categoryFeeder = listFeeder(
        List.of(
            Map.of("category", "web-development", "sort", "popular"),
            Map.of("category", "mobile-development", "sort", "newest"),
            Map.of("category", "backend", "sort", "trending"),
            Map.of("category", "frontend", "sort", "popular"),
            Map.of("category", "devops", "sort", "newest"),
            Map.of("category", "ai-machine-learning", "sort", "trending"),
            Map.of("category", "data-science", "sort", "popular"),
            Map.of("category", "cybersecurity", "sort", "newest"),
            Map.of("category", "cloud-computing", "sort", "trending"),
            Map.of("category", "databases", "sort", "popular")
        )
    ).random();

    // Scenario definition
    private ScenarioBuilder scn = scenario("Viblo Category Filter Test - " + USER_COUNT + " Users")
        .feed(categoryFeeder)
        .exec(
            http("Load Homepage")
                .get("/")
                .check(status().is(200))
        )
        .pause(Duration.ofSeconds(1, 2))
        .exec(
            http("Browse Posts")
                .get("/posts")
                .check(status().is(200))
                .check(responseTimeInMillis().lte(5000))
        )
        .pause(Duration.ofSeconds(2, 3))
        .exec(
            http("Filter by Category: #{category}")
                .get("/posts?category=#{category}")
                .check(status().is(200))
        )
        .pause(Duration.ofSeconds(2, 4))
        .exec(
            http("Sort by #{sort} in #{category}")
                .get("/posts?category=#{category}&sort=#{sort}")
                .check(status().is(200))
        )
        .pause(Duration.ofSeconds(1, 3))
        .exec(
            http("Load Page 2 of #{category}")
                .get("/posts?category=#{category}&sort=#{sort}&page=2")
                .check(status().is(200))
        )
        .pause(Duration.ofSeconds(2, 3))
        .exec(
            http("Filter by Time Range")
                .get("/posts?category=#{category}&time=week")
                .check(status().is(200))
        )
        .pause(Duration.ofSeconds(1, 2))
        .exec(
            http("Browse Questions in #{category}")
                .get("/questions?category=#{category}")
                .check(status().is(200))
        );

    // Load model based on user count
    {
        if (USER_COUNT <= 50) {
            setUp(
                scn.injectOpen(
                    atOnceUsers(USER_COUNT)
                )
            ).protocols(httpProtocol)
            .maxDuration(Duration.ofMinutes(12))
            .assertions(
                global().responseTime().percentile3().lte(4000),
                global().successfulRequests().percent().gte(95.0)
            );
        } else if (USER_COUNT <= 200) {
            setUp(
                scn.injectOpen(
                    rampUsers(USER_COUNT).during(Duration.ofMinutes(3))
                )
            ).protocols(httpProtocol)
            .maxDuration(Duration.ofMinutes(18))
            .assertions(
                global().responseTime().percentile3().lte(5000),
                global().successfulRequests().percent().gte(90.0)
            );
        } else {
            setUp(
                scn.injectOpen(
                    rampUsers(USER_COUNT).during(Duration.ofMinutes(5))
                )
            ).protocols(httpProtocol)
            .maxDuration(Duration.ofMinutes(25))
            .assertions(
                global().responseTime().percentile3().lte(6000),
                global().successfulRequests().percent().gte(85.0)
            );
        }
    }
}
